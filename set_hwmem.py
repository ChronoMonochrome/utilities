...
def usage():
	print("""usage: ./set_hwmem.py input_file hwmem_size, e.g.
./set_hwmem.py boot.img 74
hwmem_size should be between 1 and 127 MB""")
	exit()
...
if (len(sys.argv) < 3):
	...
	usage()
elif (int(sys.argv[2]) < 1 or int(sys.argv[2]) > 127):
	usage()
...
def repack_kernel(input_file):
	...
	if (ZIMAGE_START >= 0): 
		...
		open(zimage_file,'wb').write(bootimg[ZIMAGE_START:-ZIMAGE_CHUNK_END_SIZE])
	...
	image_chunk_start = image.find("hwmem=")
	test_pattern = image.find("M@256M")
	if (image_chunk_start == -1 or test_pattern == -1):
		debug_print('error, specific patch is not found in kernel')
		return -1

	custom_param = get_custom_cmdline(int(sys.argv[2]))
	...
	open(DIR + "boot_mod.img", "wb").write(
                      ...
                      open(DIR + "kernel_mod.lz4", "rb").read() +
		      bootimg[-ZIMAGE_CHUNK_END_SIZE:] )

	debug_print('successfully repacked to boot_mod.img!')
